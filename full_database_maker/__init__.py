...
# STRINGS_FOR_TEST = ["venue:ICML: year:2019: "]
...
JOURNALS = {'TPAMI': 1979, 'TKDE': 1989, 'JMLR': 2003}
...
def get_volume_nums_by_year(journals=JOURNALS, journal_names = JOURNAL_NAMES_1):
    '''Needs error handling very bad- can break easily'''
...
def search(soup, journals = False, journals_dict = JOURNALS_DICT):
    '''Makes separate dataframe entries for each author. Condense into count here too? Yeah, let's just do a total count'''
...
def year_conf_looper(confs=CONFERENCES):
    '''Go through conf dictionary, at every conf loop through years from start year through and including present year
    Make that the string that query_db uses (style correctly)'''
...
def find_all_results_per_conf(search_string=STRINGS_FOR_TEST):
    '''If there are more than 1000 hits, start from 1001. Does not account for conferences with > 2000 entries (so far, don't exist) This fixes the cap on the API for bandwidth purposes
    Returns final dataframe for a single year at a conference. Eventually generalize to > 2000 with a modulo/division while loop'''
...
def setup_main_df(confs=CONFERENCES):
    '''create the empty dataframe with headers relating to conferences'''
...
def read_in_df_from_file(file_name='/Users/samski/Downloads/hi4.csv'):
...
def postprocess_df(df, venues=CONFERENCES, journals = False, journals_dict = JOURNALS_DICT):
...
def join_frames(confs_frame, journal_frame, venues=CONFERENCES, journals = False, journals_dict = JOURNALS_DICT):
    ...
    columns_list = JOURNAL_NAMES_1 #MAKE THIS WHOLE THING INTO HELPER FUNCTION
...
def consolidate_names(df, venues=CONFERENCES):
